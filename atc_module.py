...
def show_gate_management(parent, switch_page, back_page, back_args):
    ...
    container = tk.Frame(parent, bg=CARD_COLOR, width=950, height=350)
    container.pack(padx=20, pady=10, fill="both", expand=True)
    container.pack_propagate(False)

    canvas = tk.Canvas(container, bg=CARD_COLOR, highlightthickness=0, width=950, height=350)
    ...
    tk.Label(header_frame, text="Actions", font=("Segoe UI", 10), fg=TEXT_COLOR, bg="#475569", width=22).pack(side="left")
...
def show_runway_management(parent, switch_page, back_page, back_args):
    ...
    container = tk.Frame(parent, bg=CARD_COLOR, width=950, height=300)
    container.pack(padx=20, pady=10, fill="both", expand=True)
    container.pack_propagate(False)

    canvas = tk.Canvas(container, bg=CARD_COLOR, highlightthickness=0, width=950, height=300)
    canvas.pack(side="left", fill="both", expand=True)
    scrollbar = tk.Scrollbar(container, orient="vertical", command=canvas.yview)
    scrollbar.pack(side="right", fill="y")
    canvas.configure(yscrollcommand=scrollbar.set)
...
def show_flight_schedule(parent, switch_page, back_page, back_args):
    ...
    tk.Label(parent, text="Flight Schedule Operations", font=("Segoe UI", 22, "bold"), fg=TEXT_COLOR, bg=BG_COLOR).pack(pady=20)

    # Create a frame for the list (scrollable)
    container = tk.Frame(parent, bg=CARD_COLOR, width=1200, height=350)
    container.pack(padx=20, pady=10, fill="both", expand=True)
    container.pack_propagate(False)

    canvas = tk.Canvas(container, bg=CARD_COLOR, highlightthickness=0, width=1200, height=350)
    canvas.pack(side="left", fill="both", expand=True)
    scrollbar = tk.Scrollbar(container, orient="vertical", command=canvas.yview)
    scrollbar.pack(side="right", fill="y")
    canvas.configure(yscrollcommand=scrollbar.set)
...
def show_flight_status(parent, switch_page, back_page, back_args):
    ...
    tk.Label(header_frame, text="Manual Override", font=("Segoe UI", 10), fg=TEXT_COLOR, bg="#475569", width=25).pack(side="left")
    ...
    for flight in flights:
        ...
        status_options = ["SCHEDULED", "CHECK-IN OPEN", "BOARDING", "GATE CLOSED", "TAXIING", "DEPARTED", "IN AIR", "ARRIVED", "DELAYED", "CANCELLED"]
        status_var = tk.StringVar(value=flight.get('status', '').upper())
        status_menu = tk.OptionMenu(row_frame, status_var, *status_options,
                                    command=lambda val, fid=flight['flight_id']: update_flight_status(fid, val, switch_page, show_flight_status, back_page, back_args))
        status_menu.config(bg=BTN_PRIMARY, fg="white", font=("Segoe UI", 9, "bold"), width=20, relief="flat", highlightthickness=0, cursor="hand2")
        status_menu["menu"].configure(bg=CARD_COLOR, fg=TEXT_COLOR, font=("Segoe UI", 9), activebackground=BTN_PRIMARY, activeforeground="white", relief="flat")
